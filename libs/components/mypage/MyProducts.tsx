...
interface InitialType  {
    ...
    limit: number;
    sort: string;
    search: { propertyStatus: string };
  };

const MyProducts: NextPage<InitialType> = (initialInput = {page: 1,  limit: 5,  sort: 'createdAt',  search: {
    propertyStatus: 'ACTIVE',
}} ) => {
	const device = useDeviceDetect();
	const [searchFilter, setSearchFilter] = useState<InitialType>(initialInput);
...
// 	});
	...
	// 		await updateProperty({ variables: { input: id, propertyStatus: 'DELETE' } },)
	// 	  }
				...
				<Stack className="property-list-box">
					...
					<Stack className="list-box">
						...
						{agentProperties?.length === 0 ? (
							...
							agentProperties.map((index, product) => {
								return (
									<PropertyCard
										key={product}
									/>
